...
function getApiPath(obj, template) {
    // const result = template.split('%').map(it => obj.hasOwnProperty(it) ? obj[it] : it).join('').replace(/\s+/g, `%${obj.id}`);
    const result = template.replace(/%(.*?)%/g, (_, prop) => obj[prop]).replace(/\s+/g, `%${obj.id}`);
    return result;
}
